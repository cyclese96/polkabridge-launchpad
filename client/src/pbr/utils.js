...
export const getProgress = async (lpAddress, pid, access, startAt, endAt, lpNetwork) => {
  try {
    ...
    const lpContract = getCurrentLaunchpadContract(lpAddress, pid, access, lpNetwork, currentNetwork);
...
export const getIsWhitelist = async (lpAddress, pid, access, stakeAmount, account, lpNetwork) => {
...
export const joinpool = async (lpAddress, pid, access, stakeAmount, ethValue, account, network, symbol) => {
...
export const getMaxAllocation = async (lpAddress, pid, access, account, network) => {
  ...
  try {
    ...
    console.log('ethTest: getMaxAllocation exception  ', { error, lpAddress, pid })
    return '0';
  }
...
export const getUserInfo = async (lpAddress, pid, access, account, network) => {
  ...
  try {
    ...
    const chainId = await getCurrentNetworkId();
    const currentNetwork = getNetworkName(chainId);
    const lpContract = getCurrentLaunchpadContract(lpAddress, pid, access, network, currentNetwork);
...
const abiMapping = {
  ...
  'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx': launchpadBscAbi3  // put battle saga  mainnet address if ABi same
}
